...
@section('rootcontent')
    <section class="wrapper">
        ...
        <div class="container">
            ...
            <br>
            <div class="row">
                {!! Form::open(['url'=>'admin/sectionStudent/update','method'=>'POST','class'=>'form-horizontal','enctype'=>'multipart/form-data']) !!}
                                ...
                                <div class="col-sm-6 {{ $errors->has('subject_id') ? ' has-error' : '' }}">
                                    ...
                                    <select class="form-control" id="sel1" name="subject_id">
                                        @foreach( $subjects as $subject)
                                            <option value="{{$subject->id}}" @if($data->subject_id==$subject->id) selected @endif>{{$subject->subjectName}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-sm-6 {{ $errors->has('student') ? ' has-error' : '' }}">
                                    <select class="form-control" id="sel1" name="student_id">
                                        @foreach( $students as $student)
                                            <option value="{{$student->id}}" @if($data->student_id==$student->id) selected @endif>{{$student->name}}</option>
                                        @endforeach
                                    </select>
